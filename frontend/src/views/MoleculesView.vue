  ...
  <div class="items-view">
    ...
    <div class="items-grid" v-if="filtered.length">
      ...
      >
        ...
        <div class="tile-info">
          <div class="tile-meta">
            <span class="badge">Molécule</span>
    ...
    <div v-else class="empty-state">
      <p v-if="search">Aucune molécule correspondant à « {{ search }} ».</p>
      <p v-else>Aucune molécule créée.</p>
    </div>

    <!-- Create Modal -->
    <div class="modal-overlay" v-if="showCreate" @click.self="showCreate = false">
      <div class="modal">
        <h3>Nouvelle molécule</h3>
...
const {
  ...
  (item) => item.name,
  async (item, name) => {
    const updated = await api.patchMolecule(item.id, { name })
...
function openEditor(item) {
  ...
  router.push(`/molecules/${item.id}/editor`)
}
...
function molWidth(item) {
  return item.width_mm || item.definition?.width_mm || 30
}
function molHeight(item) {
  return item.height_mm || item.definition?.height_mm || 20
}
...
function thumbStyle(item) {
  const MAX = 120
  const w_mm = molWidth(item)
  const h_mm = molHeight(item)
  const ratio = h_mm / w_mm
  return { width: `${MAX}px`, height: `${Math.round(MAX * ratio)}px` }
}

async function create() {
  const created = await api.createMolecule({
    ...form.value,
    definition: { width_mm: form.value.width_mm, height_mm: form.value.height_mm, layers: [], dataSchema: {} },
  })
...
async function duplicate(item) {
  const dup = await api.duplicateMolecule(item.id)
...
async function remove(item) {
  ...
  await api.deleteMolecule(item.id)
