...
| Date | Résumé |
|------|--------|
| 2026-03-12 | Initialisation du plan. Features complétées : correction bug réordonnancement calques (inversion visuelle/array), persistance état expanded localStorage, icône cadenas SVG monochrome, zone dépôt bas de liste. Création `specs/README.md`, `specs/WORKPLAN.md`, `CLAUDE.md`. Création des 10 TSDs complets : TSD-001 (canvas), TSD-002 (calques), TSD-003 (atomes), TSD-004 (médias), TSD-005 (listing), TSD-006 (recto/verso), TSD-007 (instances), TSD-008 (molécules), TSD-009 (export), TSD-010 (validation). |
