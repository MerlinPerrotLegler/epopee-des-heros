...
| Date | Résumé |
|------|--------|
| 2026-07-17 (2) | Fix médias « local vs distant » : `.env` chargé depuis la racine (`loadEnv.js`, indépendant du cwd) ; en mode MySQL, médias **uniquement** en BLOB distant (pas de cache disque local) ; Vite proxy lit `PORT` depuis `.env` au lieu d’un port codé en dur. |
