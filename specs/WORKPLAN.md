...
- [ ] **Refonte affichage mm** : carte en CSS `mm` + viewport `translate`/`scale` (plus de `mm×zoom→px`) — design + plan prêts, implémentation à faire
...
- [ ] TSD rédigé (TSD-009) ✓
- [ ] Impression navigateur : carte seule à scale 1, multi-cartes `break-inside: avoid` (dépend refonte CSS mm)
...
- [ ] Pas de longueur carte en `px` dérivé de `mm×zoom` — rendu CSS `mm` + zoom viewport (refonte en cours)
...
1. **[Phase 1/4]** Implémenter refonte CSS mm + viewport `scale`/`translate` (`docs/superpowers/plans/2026-07-18-mm-css-viewport-transform.md`) — corrige dérive texte au zoom + base print
2. **[Phase 4]** Print navigateur multi-cartes (`break-inside: avoid`) une fois le rendu mm en place
