...
router.get('/users', (req, res) => {
  const db = getDb()
  const rows = db.prepare('SELECT id, username, role, created_at, updated_at FROM users ORDER BY username').all()
  res.json(rows)
})
...
router.post('/users', (req, res) => {
  ...
  try {
    ...
    const row = db.prepare('SELECT id, username, role, created_at, updated_at FROM users WHERE id = ?').get(id)
...
router.patch('/users/:id', (req, res) => {
  ...
  const row = db.prepare('SELECT id, username, role, created_at, updated_at FROM users WHERE id = ?').get(req.params.id)
...
router.delete('/users/:id', (req, res) => {
  ...
  db.prepare('DELETE FROM layout_locks WHERE user_id = ?').run(req.params.id)
