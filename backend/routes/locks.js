...
async function cleanExpired(db) {
  await db.prepare('DELETE FROM layout_locks WHERE expires_at < ?').run(Date.now())
}
 ...
 */
router.post('/layouts/:layoutId/acquire', async (req, res) => {
  const db = getDb()
  await cleanExpired(db)
  const layoutId = req.params.layoutId
  const layout = await db.prepare('SELECT id FROM layouts WHERE id = ?').get(layoutId)
  ...
  const existing = await db.prepare('SELECT * FROM layout_locks WHERE layout_id = ?').get(layoutId)
  if (existing) {
 ...
 */
router.post('/layouts/:layoutId/heartbeat', async (req, res) => {
  const db = getDb()
  const row = await db.prepare('SELECT * FROM layout_locks WHERE layout_id = ?').get(req.params.layoutId)
  ...
  await db.prepare('UPDATE layout_locks SET expires_at = ? WHERE layout_id = ?').run(exp, req.params.layoutId)
...
router.delete('/layouts/:layoutId', async (req, res) => {
  ...
  await db.prepare('DELETE FROM layout_locks WHERE layout_id = ?').run(req.params.layoutId)
  res.json({ ok: true })
})
